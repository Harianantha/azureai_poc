...
class HandleOCR:
    ...
    def translateImage(self,argv,pageVal):
        # The URL of a JPEG image containing text.
        print ("Performing ocr on file:%s" %argv)
        ...
        try:
            ...
            conn = http.client.HTTPSConnection(HandleOCR.uri_base)
            #conn.request("POST", "/vision/v1.0/ocr?%s" % params, body, headers)
            conn.request("POST", "/vision/v1.0/ocr?%s" % HandleOCR.params, image, HandleOCR.headers)
        ...
        #    print("Number of lines %s" %len(lines))
            for region in regions:
                ...
                for line in lines:
                    ...
                    for word in words:
                            #print(word["text"])
                            translatedtText=self.translatetext(word["text"])

                            boundingBoxVals=word["boundingBox"].split(',')
                            boundingBox = []
                            width = int(boundingBoxVals[2])
                            height = int(boundingBoxVals[3])
                            startX = int(boundingBoxVals[0])
                            startY = int(boundingBoxVals[1])

                            boundingBox.append( startX)
                            boundingBox.append( startY)

                            boundingBox.append(startX + width)
                            boundingBox.append(startY)

                            boundingBox.append(startX + width)
                            boundingBox.append( startY + height)

                            boundingBox.append( startX)
                            boundingBox.append(startY + height)
                            #print('Before create OCRResponseWord %s'%boundingBox)
                            ocrobj = OCRResponseWord (word["text"],boundingBox)
                            #print('After create OCRResponseWord %s' % boundingBox)
                            pageVal.createOrAddToPageRow(ocrobj, translatedtText, 'OCR')
    ...
    def translatetext (self,textVal):
        ...
        headers = {'Ocp-Apim-Subscription-Key': HandleOCR.translation_subscription_key}
        conn = http.client.HTTPSConnection(HandleOCR.translation_host)
        params = '?to=en&category=generalnn&text=' + urllib.parse.quote (textVal)

        conn.request ("GET", HandleOCR.translation_path + params, None, headers)
        response = conn.getresponse ()
